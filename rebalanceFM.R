...
if(stocksToBuy>=0 & stocksToBuy <1){
return(stocksToBuy)
}
...
else if (stocksToBuy>=1) {
return(1) #kup co sa da
}
}
else {
return(0)
...
if(stocksToSell<0){
#nepredavaj nic, rozdiel je maliky
return(0)
}
else if (stocksToSell >= 0 & stocksToSell < 1) {
return(-stocksToSell)
#predaj tolko co ti odporuci
}
else if(stocksToSell >= 1){
#predaj vsetko
return(-1)
}
}
...
[Yesterday 20:24] Gibasová Natália
...
result <- pseudoRebalance(knownData,c(upLine,bottomLine),knownData,knownDegrees,aoStocks) #pseudoRebalance nam v najaktualnejsie dato v knownData povie kolko kupit/predat
#print(result)
if(result>0){
#kupuj
result <- result*aoStocks*(1-fees) # fees z kupi odidu prec
aoStocks <- aoStocks + result
money <- money - result*unknownAdjBuy[i] # kupuj za cenu v ten den
}
else {
result <- -result*aoStocks*(1-fees) #fees z predaja, iba cast mozes predavat
aoStocks <- aoStocks + result
...
profits <-c( profits, ((money+aoStocks*unknownAdjClose[length(unknownAdjClose)])-beginingVal)) #vektor profitov pre jednu akciu, pri rozne nadstavenych hraniciach
...
if(meanVector[length(meanVector)-1]< criticalUpLine){
stocksToSell <- 1 - (degreesUp/ atan(meanVector[length(meanVector)]-meanVector[length(meanVector)-1]))
if(stocksToSell<=0 ){
#nic nepredavame
...
stocksToSell <- floor(aoStocks[index]*stocksToSell) #preda od 0 po 1 nasobok toho co mas
budget= budget + stocksToSell*latestSellPrice*(1-fees) #predavame za low cenu plus poplatky
print( paste("SELL", names[index],", amount: " , stocksToSell ,sep=" "))
}
else {
stocksToSell <- aoStocks[index] #predaj vsetko
budget= budget + stocksToSell*latestSellPrice*(1-fees) #predavame za low cenu plus poplatky
print( paste("SELL", names[index],", amount: " , stocksToSell ,sep=" "))
}
}
...
stocksToBuy <- 1 - (degreesDown/ atan(meanVector[length(meanVector)]-meanVector[length(meanVector)-1]))
if(stocksToBuy>0 & stocksToBuy <= 1){
if(budget-(floor(stocksToBuy*aoStocks[index])*latestBuyPrice*(1+fees)) >=0){
budget= budget - floor(stocksToBuy*aoStocks[index])*latestBuyPrice*(1+fees)
print(paste("BUY",names[index], ", amount:", floor(stocksToBuy*aoStocks[index]) ,sep=" "))
}
else {
while(budget - (floor(stocksToBuy*aoStocks[index])-num)*latestBuyPrice*(1+fees)<0 & num<(floor(stocksToBuy*aoStocks[index]))-1){
num = num + 1
}
if(budget - (floor(stocksToBuy*aoStocks[index])-num)*latestBuyPrice*(1+fees)>=0){
print(paste("BUY",names[index], ", amount:", (floor(stocksToBuy*aoStocks[index])-num) ,sep=" "))
budget = budget - (floor(stocksToBuy*aoStocks[index])-num)*latestBuyPrice*(1+fees)
}
else {
print(paste("Not enough funds to buy", (floor(stocksToBuy*aoStocks[index])-num) , "for", names[index], sep=" " ))
}
}
}
else if(stocksToBuy > 1) {
if(budget - aoStocks[index]*latestBuyPrice*(1+fees)>=0){
budget= budget - aoStocks[index]*latestBuyPrice*(1+fees)
print(paste("BUY",names[index], ", amount:", aoStock[index] ,sep=" "))
}
else {
num <- 1
while(budget - (aoStocks[index]-num)*latestBuyPrice*(1+fees)<0 | num!=aoStocks[index]){
num = num + 1
}
if(budget - (aoStocks[index]-num)*latestBuyPrice*(1+fees)>=0){
print(paste("BUY",names[index], ", amount:", floor(aoStocks[index]-num) ,sep=" "))
budget = budget - (aoStocks[index]-num)*latestBuyPrice*(1+fees)
}
else {
print(paste("Not enough funds to buy", floor(aoStocks[index]-num) , "for", names[index], sep=" " ))
}
}
}
else if (stocksToBuy<0) {
